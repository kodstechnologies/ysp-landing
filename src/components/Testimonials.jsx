    ...
    <section className="py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-20">
          ...
          <h2 className="text-3xl sm:text-5xl font-black text-deep-blue mb-6 tracking-tight">How our <span className="text-lochmara">Users Buy Fuel</span></h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {testimonials.map((t, i) => (
            <div key={i} className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/40 hover:shadow-2xl transition-all duration-300 relative group overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-orange-50 rounded-bl-[5rem] -mr-16 -mt-16 group-hover:scale-110 transition-transform duration-500"></div>
              
              <div className="relative mb-8 flex items-center justify-between">
                <div className="flex gap-1">
                  {[...Array(t.stars)].map((_, si) => <Star key={si} size={14} className="fill-golden-yellow text-golden-yellow" />)}
                </div>
                <span className="text-[10px] font-black text-lochmara uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full">{t.step}</span>
              </div>

              <div className="mb-8 p-4 bg-slate-50 w-16 h-16 rounded-2xl flex items-center justify-center text-vibrant-orange">
                 {t.icon}
              </div>
              ...
              <div className="flex items-center gap-4 pt-6 border-t border-slate-100">
                <div className="w-12 h-12 rounded-full bg-lochmara flex items-center justify-center text-white font-black text-lg">{t.name.charAt(0)}</div>
                <div>
                  <h4 className="text-deep-blue font-black tracking-tight">{t.name}</h4>
